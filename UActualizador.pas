...
begin
  ...
  begin
    ...
    Registro.CloseKey;
       ...
       Label2.Caption:= 'No se pudo config. el auto-inicio, de la utilidad de ' +
       'actualizacion automatica de Easy System';
  end;
...
begin
  ...
  PonerProgramaInicio;
  Ini4.WriteString('Fechas', 'FechaInicio1', '21');
  FechaInicio1:= Ini4.ReadString ('Fechas', 'FechaInicio1', '');
  FechaInicio2:= Ini4.ReadString ('Fechas', 'FechaInicio2', '');
  if FechaInicio2 = '' then
    begin
      Ini4.WriteString('Fechas', 'FechaInicio2', '0');
    end
    else
    begin
      if FechaInicio1 <> FechaInicio2 then
    begin
      TmInicioWindows.Enabled:= True;
      Ini4.WriteString('Fechas', 'FechaInicio2', FechaInicio1 );
      Ini4.Free;
    end
    else
    begin
...
Label2.Caption:= 'Intento de conexion fallida (AutoAct).';
TmReiniciar.Enabled:= True;
Exit;
end;
...
Update;
  ...
  try
   ...
   on E: Exception do ShowMessage(E.Message + ' 1 '+IntToStr(E.HelpContext));
    //raise Exception.Create( 'No se ha podido conectar con el servidor. ');
    //Label2.Caption:= 'No se ha podido conectar con el servidor (AutoAct1). ';
    //Update;
...
Gauge1.MaxValue:= Barra;}
  for i := 0 to Afiles2.Count -1 do begin
   ...
   try
    ...
    FTP2.Get( ExtractFileName(AFiles2[i]), 'D:\Easy System S2010\Actualizador\'+AFiles2[i],True);
    Sleep(500);
    Except

      //raise Exception.Create( 'No se ha podido iniciar la descarga. ');
      //Label2.Caption:= 'No se ha podido iniciar la descarga (AutoAct2). ';
      //Update;
      on E: Exception do ShowMessage(E.Message + ' 2 '+IntToStr(E.HelpContext));
      //TmReiniciar.Enabled:= True;
      //Exit;
      //PararEjecucion:= True;
      //Timer3.Enabled:= True;
